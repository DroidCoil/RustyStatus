...
pub fn configs() -> Result<Vec<Module>> {
    Ok(vec![
        ...
        create_module(1, || batterystat().unwrap_or_else(|e| format!("Battery error: {}", e))),
        create_module(1, || volumestat().unwrap_or_else(|e| format!("Volume error: {}", e))),
        create_module(1, || ramstat().unwrap_or_else(|e| format!("RAM error: {}", e))),
    ])
}
